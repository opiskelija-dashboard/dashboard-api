class HttpHelpers
  ...
  def self.tmc_api_get(endpoint, access_token = nil, base_address = nil)
    base_address = Rails.configuration.tmc_api_base_address if base_address.nil?
    ...
    if response.code.to_s == '200'
      ...
      Rails.logger.debug
      ("do_tmc_api_get: GET to #{full_api_call_address} returned code
      #{response.code.inspect} (!= 200). Response body: #{response.body}")
